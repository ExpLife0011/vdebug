...
class StyleContext : public StyleContextBase {
	LexAccessor &styler;
...
public:
	...
	virtual void Forward(int nb) {
		for (int i = 0; i < nb; i++) {
			Forward();
		}
	}

	virtual void ForwardBytes(int nb) {
	...
	virtual void ChangeState(int state_) {
		state = state_;
	}

	virtual void SetState(int state_) {
		styler.ColourTo(currentPos - ((currentPos > lengthDocument) ? 2 : 1), state);
		state = state_;
	}

	virtual void ForwardSetState(int state_) {
		Forward();
		styler.ColourTo(currentPos - ((currentPos > lengthDocument) ? 2 : 1), state);
		state = state_;
	}

	virtual int LengthCurrent() const {
		return currentPos - styler.GetStartSegment();
	}

	virtual int GetRelative(int n) {
		return static_cast<unsigned char>(styler.SafeGetCharAt(currentPos+n, 0));
	}

	virtual int GetRelativeCharacter(int n) {
		if (n == 0)
			return ch;
		if (multiByteAccess) {
	...
	virtual bool Match(char ch0) const {
		return ch == static_cast<unsigned char>(ch0);
	}

	virtual bool Match(char ch0, char ch1) const {
	...
	virtual bool MatchIgnoreCase(const char *s) {
	...
	virtual bool MatchIgnoreCase2(const char *s) {
	...
	// Non-inline
	virtual void GetCurrent(char *s, unsigned int len);
	virtual void GetCurrentLowered(char *s, unsigned int len);
};
