...
int md5_buffer(const char* buffer, int bufSize, char* outBuf, int outSize)
{
    ...
    md5_init(&context);
    md5_update(&context, (unsigned char*)buffer, bufSize);
...
static int _md5_file(FILE* file, char* outBuf, int outSize)
{
    MD5_CTX context;
    int i, len;
...
int md5_file(const char* fileName, char* outBuf, int outSize)
{
    ...
    ret = _md5_file(file, outBuf, outSize);
...
int md5_file16(const wchar_t* fileName, char* outBuf, int outSize)
{
    ...
    if (0 == (file = _wfopen(fileName, L"rb")))
    {
        return 0;
    }
